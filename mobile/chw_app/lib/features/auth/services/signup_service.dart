...
class AuthService {
  ...
  Future<Map<String, dynamic>> signInWithGoogle() async {
    try {
      ...
      if (isNewUser) {
        ...
        // Log signup
        await _logUserSignup(user.uid, user.email ?? '', 'CHW');
        ...
        if (userData['status'] == 'Deactivated') {
          ...
          return {
            "success": false,
            "error": "Your account has been deactivated by administrator. Please contact admin to restore access."
          };
        }

        // Check if user has CHW role
        if (userRole != "CHW") {
          await _auth.signOut();
          return {
            "success": false,
            "error": "This account is not registered as a Community Health Worker. Please use the appropriate app."
          };
        }
  ...
  /// Sign up user with validation (Email/Password)
  Future<String?> signUp({
    ...
    try {
      ...
      // ✅ Get current city (use detectedCity if provided)
      String? city = detectedCity ?? await _getCurrentCity();
