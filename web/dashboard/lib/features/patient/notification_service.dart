...
import 'notification_model.dart'; // Import notification model
...
class NotificationService {
  ...
  // Save token to Firestore - UPDATED FOR PATIENT
  Future<void> _saveTokenToFirestore(String token) async {
    try {
      ...
      if (user != null) {
        ...
        // Also verify in patients collection if role is Patient
        if (userRole == 'Patient') {
          final verifyPatient = await _firestore.collection('patients').doc(userId).get();
          if (verifyPatient.exists) {
            final patientToken = verifyPatient.data()?['fcmToken'];
            print('🔍 VERIFICATION - Token in patients collection: ${patientToken != null ? '✅ Present' : '❌ Missing'}');
          }
        }
