...
class CHWDashboardService {
  ...
  /// 🔥 NEW: Check if doctor added requestedTests in diagnosis
    ...
    return _assignedPatients.snapshots().asyncMap((snapshot) async {
      ...
      for (var patientDoc in snapshot.docs) {
        ...
        if (chwScreeningSnap.docs.isNotEmpty) {
          ...
          // 🔥 GET SELECTED DOCTOR FROM SCREENING
          assignedDoctorId = firstDoc.data()['assignedDoctorId'];
          assignedDoctorName = firstDoc.data()['assignedDoctorName'];
        }
        ...
        if (screeningId != null && screeningId.isNotEmpty) {
          ...
          if (patientScreeningSnap.exists) {
            ...
            // 🔥 GET SELECTED DOCTOR FROM PATIENT SCREENING IF NOT ALREADY SET
            if (assignedDoctorId == null) {
              assignedDoctorId = patientScreeningData?['assignedDoctorId'];
              assignedDoctorName = patientScreeningData?['assignedDoctorName'];
            }
        ...
        // 4️⃣ Check main /patients/{patientId} doc for top-level status and selectedDoctor
        final mainPatientDoc =
        await _firestore.collection('patients').doc(patientId).get();
        if (mainPatientDoc.exists) {
          final mainData = mainPatientDoc.data();
          if (mainData?['status'] != null && mainData!['status'] != status) {
            status = mainData['status'];
          }

          // 🔥 GET SELECTED DOCTOR FROM MAIN PATIENT DOC
          if (assignedDoctorId == null) {
            assignedDoctorId = mainData?['selectedDoctor'];
            // If we have doctor ID but no name, try to fetch doctor name
            if (assignedDoctorId != null && assignedDoctorId!.isNotEmpty && assignedDoctorName == null) {
              final doctorDoc = await _firestore.collection('doctors').doc(assignedDoctorId).get();
              if (doctorDoc.exists) {
                assignedDoctorName = doctorDoc.data()?['name'] ?? 'Unknown Doctor';
              }
            }
          }
        }
    ...
    return _assignedPatients.snapshots().asyncMap((snapshot) async {
      ...
      for (var patientDoc in snapshot.docs) {
        ...
        if (screeningSnapshot.docs.isNotEmpty) {
          ...
          assignedDoctorName = screeningData['assignedDoctorName'];
  ...
  Future<PatientWithScreening?> getPatientById(String patientId) async {
    try {
      ...
      if (mainDoc.exists) {
        ...
        // 🔥 GET SELECTED DOCTOR FROM MAIN PATIENT DOC IF NOT ALREADY SET
        if (assignedDoctorId == null) {
          assignedDoctorId = mainData?['selectedDoctor'];
          if (assignedDoctorId != null && assignedDoctorId!.isNotEmpty && assignedDoctorName == null) {
            final doctorDoc = await _firestore.collection('doctors').doc(assignedDoctorId).get();
